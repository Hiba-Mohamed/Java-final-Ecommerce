    ...
    public static void displayLoginRegisterMenu(UserService userService) {
        while (true) {
            ...
            if (choice == 1) {
                ...
                if (roleChoice == 1) {
                    User newUser = new Buyer(username, email, password); // Create a Buyer object
                    userService.addUser(newUser);
                    System.out.println("Registration successful as a Buyer. Welcome, " + username);
                    loggedInUser = username;
                    showBuyerMenu(); // Show the Buyer menu
                } else if (roleChoice == 2) {
                    User newUser = new Seller(username, email, password); // Create a Seller object
                    userService.addUser(newUser);
                    System.out.println("Registration successful as a Seller. Welcome, " + username);
                    loggedInUser = username;
                    showSellerMenu(); // Show the Seller menu
                } else {
